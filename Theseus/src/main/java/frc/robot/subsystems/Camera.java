...
public class Camera {
    ...
    public record datatatatata(double targetYaw, double targetPitch, double targetArea, Transform3d alternatePose, double dumbdistance, Pose3d robotpose, Translation2d translation2d, PhotonPipelineResult result) {
    }
    ...
    public datatatatata cameraProcessing(PhotonCamera camera) {
        ...
        if (result.hasTargets()) {
            ...
            // LeBron found out that reprojection error means the distance between a target
            // and the best location for the target to be (the rightmost side)
            ...
            if (aprilTagFieldLayout.getTagPose(rightmostTarget.getFiducialId()).isPresent() && cameraToRobot != null) {
                robotpose = PhotonUtils.estimateFieldToRobotAprilTag(
                    rightmostTarget.getBestCameraToTarget(),
                    aprilTagFieldLayout.getTagPose(rightmostTarget.getFiducialId()).get(), 
                    cameraToRobot
                );
            }
